...
public class BigComplex
{
	...
	{
		return BigDecimalOper.sqrt(norm(), mc);
	}
	...
	BigComplex sqrt(MathContext mc)
	{
		...
		if ( l.compareTo(BigDecimal.ZERO) == 0 ) {
			return new BigComplex(
					BigDecimalOper.scalePrec(BigDecimal.ZERO, mc),
					BigDecimalOper.scalePrec(BigDecimal.ZERO, mc));
		}
		final BigDecimal u = BigDecimalOper
				.sqrt(l.add(re).divide(half, mc), mc);
		final BigDecimal v = BigDecimalOper.sqrt(l.subtract(re)
				.divide(half, mc), mc);
